...
MAIN_TEXT_PATH = os.path.join(MAIN_DIR, "body_cleaned.txt")  # <--- fixed
SUPP_PATH = os.path.join(MAIN_DIR, "supplementary.txt")
...
def split_sections(text: str) -> List[str]:
    """
    Splits text into sections based on headings like "Section X", "Figure S1", etc.
    """
    import re
    parts = re.split(r"\n\s*(?:Section|Figure|Supplementary|S\d+)[^\n]*\n", text)
...
def summarize_supplementary():
    ...
    supp_text = read_file(SUPP_PATH)
    if not supp_text.strip():
        print("⚠️ No supplementary file found.")
        return {}

    sections = split_sections(supp_text)
    summaries = {}
    ...
    for s_idx, section in enumerate(sections, start=1):
        section_chunks = sliding_chunks(section, CHUNK_SIZE_SUPP, CHUNK_OVERLAP_SUPP)
        chunk_summaries = []
...
{chunk[:4000]}"""
            ...
            chunk_summaries.append(summary_resp)
