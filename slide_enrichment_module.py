...
def _split_slides(plan_text: str):
    ...
    for i in range(1, len(parts)):
        ...
        slides.append((header + content).strip())
    return slides
...
def enrich_presentation_plan(
    ...
    *,
    ...
    ----------
    summaries_dir : str
        Directory containing: supplementary_sections.json, Bugsummary.txt or 03_main_contextualized.txt
    plan_path : str or Path
        Path to the existing plan (e.g. 04_presentation_plan.txt)
    output_dir : str or Path
        Where to save: slides_final/slide_001_final.txt + plan_final.txt
    llm : BaseLanguageModel, optional
        Any LangChain LLM (recommended: same as used before)
    model : str
